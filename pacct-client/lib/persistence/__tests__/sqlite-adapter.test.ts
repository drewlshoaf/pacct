...
describe('SQLiteAdapter', () => {
  ...
  describe('collection operations', () => {
    ...
    it('data persists across operations within same adapter', async () => {
      await adapter.put('items', { id: 'a', name: 'First', status: 'ok' });
      await adapter.put('items', { id: 'b', name: 'Second', status: 'ok' });
      await adapter.deleteById('items', 'a');

      const remaining = await adapter.getAll<TestItem>('items');
